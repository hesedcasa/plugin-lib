...
function buildDynamicFlags(fields: FieldDef[]) {
  return Object.fromEntries(
    fields.map(({char, default: def, description, name, type}) => {
      const base = {char, description}
      if (type === 'number') return [name, Flags.integer({...base, ...(def !== undefined && {default: Number(def)})})]
      if (type === 'boolean') return [name, Flags.boolean({...base, ...(def !== undefined && {default: Boolean(def)})})]
      return [name, Flags.string({...base, ...(def !== undefined && {default: String(def)})})]
    }),
  )
}
...
async function promptFieldValue(
  ...
  const {default: def, description, required, type} = f
  ...
  try {
    ...
    const raw = await input({
      ...
      message: description + ':',
      required,
      ...(currentValue !== undefined && {prefill: 'tab' as const}),
    })
...
export function createAuthAddCommand<T = AuthConfig>(options: AuthCommandOptions<T>): typeof Command {
  ...
  return class AuthAdd extends AuthCommandBase {
    ...
    static override flags = buildDynamicFlags(resolvedFields)
